...
HELP_TEXT = (
    ...
    "/upload monthly    — выгрузить месяц в Google Sheets\n"
    "/backfill          — ретро-сбор исторических данных\n"
    "/import_paid       — загрузить платные размещения из файла (xlsx/csv)\n"
    "\n"
    "🔍 Диагностика:\n"
    "/inspect @channel msg_id — сырая структура конкретного поста из Telegram\n"
    "\n"
    "⚙️ Служебное:\n"
    "/restart           — перезапустить бота (подхватить изменения в файлах)\n"
...
def register_command_handler(client: TelegramClient):
    ...
    async def handle_inspect(event):
        """
        /inspect @channel msg_id — сырая диагностика поста через УЖЕ
        работающее соединение бота (не создаёт новое подключение к
        Telegram, поэтому не подвержено обрывам, которые бывают у
        отдельно запускаемых диагностических скриптов).
        """
        sender_id = event.sender_id
        log.info(f"/inspect получено от {sender_id} (разрешённые: {ALLOWED_SENDERS})")
        if sender_id not in ALLOWED_SENDERS:
            await event.reply(f"⛔ Ваш ID {sender_id} не в списке разрешённых")
            return
        parts = event.raw_text.strip().split()
        if len(parts) != 3:
            await event.reply("Использование: /inspect @channel msg_id\nНапример: /inspect @metaclass 428")
            return
        _, channel, msg_id_str = parts
        try:
            msg_id = int(msg_id_str)
        except ValueError:
            await event.reply("msg_id должен быть числом")
            return

        log.info(f"/inspect {channel} {msg_id} от {sender_id}")
        await event.reply("⏳ Смотрю...")
        try:
            entity = await client.get_entity(channel)
            msg = await client.get_messages(entity, ids=msg_id)
            if msg is None:
                await event.reply(f"Сообщение {msg_id} не найдено (удалено?)")
                return
            ...
            lines = [
                ...
                f"media type: {type(msg.media).__name__ if msg.media else None}",
                "",
                "--- msg.media ---",
                msg.media.stringify() if msg.media else "(нет media)",
                "",
                "--- Соседние сообщения (id ±6) ---",
            ]
            async for m in client.iter_messages(entity, min_id=msg_id - 6, max_id=msg_id + 6, limit=20, reverse=True):
                marker = "  <-- ЗАПРОШЕННОЕ" if m.id == msg_id else ""
                mtype = type(m.media).__name__ if m.media else "нет media"
                text_preview = (m.message or "")[:40]
                lines.append(f"id={m.id} grouped_id={m.grouped_id} media={mtype} text={text_preview!r}{marker}")

            full_text = "\n".join(lines)
            log.info(f"/inspect результат для {channel}/{msg_id}:\n{full_text}")

            # Telegram режет длинные сообщения — если не влезает, шлём файлом
            if len(full_text) > 3500:
                import tempfile
                tmp = Path(tempfile.gettempdir()) / f"inspect_{channel.lstrip('@')}_{msg_id}.txt"
                tmp.write_text(full_text, encoding="utf-8")
                await client.send_file(sender_id, str(tmp), caption=f"📎 Диагностика {channel}/{msg_id}")
            else:
                await event.reply(f"```\n{full_text}\n```")
        except Exception as e:
            await event.reply(f"❌ Ошибка: {e}")
            log.error(f"Ошибка /inspect {channel} {msg_id}: {e}", exc_info=DEBUG_MODE)
