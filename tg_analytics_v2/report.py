...
async def build_and_send(report_type: str, debug_override: bool = False,
    ...
    async def _execute(client):
        ...
        if report_type == "monthly":
            ...
            # Пишем агрегаты в history_db.json — это и есть "база", на
            # основе которой строится Dashboard (см. get_channel_posts /
            # build_dashboard). Подписчики фиксируются один раз (не
            # переписываются при пересборке отчёта), avg_reach/err/vrpost
            # обновляются каждый раз — см. history_db.record_month().
            try:
                import history_db as _history_db
                _history_db.ensure_seeded()
                _history_db.record_month_from_report(ym, channels_data_month)
            except Exception as e:
                log.error(f"Ошибка записи в history_db: {e}")
